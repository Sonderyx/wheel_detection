...
public class SettingsMeasure extends Settings {
	...
	private final Setting[] settings = {
			...
			new SettingBool("showavg", R.string.set_show_avg, false) {
				@Override
				void onSet(boolean value) { act.setShowAvg(value); }
			},
			new SettingBool("showpalette", R.string.set_show_palette, true) {
				@Override
				void onSet(boolean value) { act.setShowPalette(value); }
			},
			new SettingBool("centraldisplay", R.string.set_centraldisplay, true) {
				@Override
				void onSet(boolean value) { act.setCentralDisplay(value); }
			},
			new SettingSliderFloat("noDigits", R.string.set_digits,1,0,3,1,1) {
				@Override
				void onSet(float i) { act.setNoDigits((int)i); }
			},
			settingDefaults
	};
}
